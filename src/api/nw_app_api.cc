...
private:
  ...
  void HandleURLProxyListener(const net::URLRequest* request,
                              const net::HostPortPair* proxy = NULL,
    ...
    if (i != url_proxy_map_.end()) {
      if (proxy == NULL && request->proxy_server().is_valid() && error >=0) {
        proxy = &request->proxy_server().host_port_pair();
      }
      if (proxy && !proxy->IsEmpty()) {
        i->second.proxy_ = proxy->ToString();
      }
      if (run_callback) {
        ...
        for (auto c : i->second.tokens_callers_) {
          ...
          results->AppendString(c.first);
          base::PostTaskWithTraits(
            FROM_HERE, {content::BrowserThread::UI},
            base::BindOnce(&DispatchEvent, "getHttpProxy", c.second,
              results));
        }
  ...
  void OnCompletedInternal(net::URLRequest* request, bool started, int net_error) override {
    HandleURLProxyListener(request, NULL, true, net_error);
  }
...
static void GetHttpProxyCallbackIO(
  ...
  base::PostTaskWithTraits(
    FROM_HERE, {content::BrowserThread::UI},
    base::BindOnce(&DispatchEvent, "getHttpProxy", caller,
      results));
}
...
bool NwAppGetHttpProxyFunction::RunNWSync(base::ListValue *response, std::string *error) {
  ...
  base::PostTaskWithTraits(
    FROM_HERE, {content::BrowserThread::IO},
    base::Bind(&GetHttpProxyCallbackIO, base::WrapRefCounted(this), base::WrapRefCounted(context_getter),
      url, token));
...
static void GetHttpAuthCallbackIO(
  ...
  base::PostTaskWithTraits(
    FROM_HERE, {content::BrowserThread::UI},
    base::BindOnce(&DispatchEvent, "getHttpAuth", caller,
      results));
}
...
bool NwAppGetHttpAuthFunction::RunNWSync(base::ListValue* response, std::string* error) {
  ...
  base::PostTaskWithTraits(
    FROM_HERE, {content::BrowserThread::IO},
    base::BindOnce(&GetHttpAuthCallbackIO, base::WrapRefCounted(context_getter),
      base::WrapRefCounted(this), gurl, realm, scheme, deep, token));
  response->AppendBoolean(true);
