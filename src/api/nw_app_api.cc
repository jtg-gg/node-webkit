...
static void DispatchEvent(
  ...
  UIThreadExtensionFunction* caller,
  base::ListValue* results) {

  scoped_ptr<base::ListValue> arguments(results);
  scoped_ptr<extensions::Event> event(
    new extensions::Event(extensions::events::UNKNOWN,
      token, arguments.Pass()));

  EventRouter::Get(caller->browser_context())->DispatchEventToExtension(
    caller->extension_id(), event.Pass());
}
...
private:
  ...
  NWJSNetworkDelegate(scoped_ptr<net::NetworkDelegate> network_delegate) :
...
public:
  ...
  static bool AddURLProxyListener(net::URLRequestContext* url_request_context, UIThreadExtensionFunction* caller,
    ...
    if(all_instance.find(network_delegate) == all_instance.end()) {
      scoped_ptr<net::NetworkDelegate> nd(network_delegate);
      network_delegate = new NWJSNetworkDelegate(nd.Pass());
...
static void GetHttpProxyCallbackIO(
  ...
  NWJSNetworkDelegate::AddURLProxyListener(rc, caller, url, token);
  ...
  content::BrowserThread::PostTask(
    content::BrowserThread::UI, FROM_HERE,
    base::Bind(&DispatchEvent, token, make_scoped_refptr(caller),
              results));
}
